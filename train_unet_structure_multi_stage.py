...
# Parse Options
path = 'RESNEXT_unet_20191029_06' + cfg.dimensional + '/'
            ...
            for epoch in range(cfg.epoch - cfg.restore_epoch):
                ...
                for i in tqdm(range((brats.train_batch * 155) // cfg.batch_size), total=((brats.train_batch * 155) // cfg.batch_size),
                    ...
                    for k in range(img_temp.shape[0]):
                        img_temp[k, :, :, :] = cv.GaussianBlur(img_temp[k, :, :, :], (0, 0), 3)
                    img_dilation = dilation.copy()
                    ...
                    if i % 5 == 0 and np.max(label_2) >= 1:
                        ...
                        # dilation_out_img
                        img_dilation_h_w = cv.Canny(img_dilation[0,:,:,0], 0, 1)
                        img_dilation_4 = np.expand_dims(img_dilation_h_w, axis=-1).repeat(4, axis=-1)
                        # edge

                        input_save = np.concatenate((img_org[0], img[0]), axis=1)
                        out_save = np.concatenate( (label_min_1[0, :, :], np.argmax(result_1[0], axis=-1),
                                                    label_min_2[0, :, :], np.argmax(result_2[0], axis=-1)
                                                    ),
                                                   axis=1)
                        ...
                        out_save = np.concatenate( (label_min_1[0, :, :], np.argmax(result_1[0], axis=-1), img_dilation[0, :, :, 0]), axis=1)
                        scp.misc.imsave(saveImgPath + brats.mhaName + str(i) + 'net_out_no_c_2.png', out_save)
                        scp.misc.imsave(saveImgPath + brats.mhaName + str(i) + 'img_dilation_2.png', img_dilation_4)
