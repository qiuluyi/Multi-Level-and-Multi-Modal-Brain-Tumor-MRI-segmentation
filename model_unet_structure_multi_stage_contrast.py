...
class model:
    def __init__(self, img, out, class_num=[2,5], class_weights =[1, 5, 3, 4, 10],
        ...
        #self.logits = self.build_multi_parallel_up_net(self.input, class_num, class_weights, train, 5)
        self.logits = self.build_multi_parallel_pyramid_and_up_net(self.input, class_num, class_weights, train, 2)
        ...
        summarized_special_variables(['stage2/layer4/redisual_layer_4/redisual_block2/conv_1/kernel:0',
                                      'stage2/layer4/redisual_layer_4/redisual_block2/conv_2/kernel:0',
                                      'stage2/layer4/redisual_layer_4/redisual_block2/conv_3/kernel:0',
                                      'stage2/updown_layer_stage2/up/conv4/kernel:0'])
    ...
    def build_net(self, inputs_org, class_num, class_weights, train=True):
        ...
        with tf.variable_scope("stage2"):
            # orgin image visualization：
            init = self.init_img(inputs_org, train)
            shape = tf.shape(inputs_org)
            self.mid_result = []
            with tf.variable_scope("layer1"):
                down_1 = redisual_block(init, 3, 64, 3, 0, train, 1, "redisual_layer_1")
                if visualization:
                    self.down_1 = down_1
            ...
            #     up_2_1 = tf.concat([up_2_1, down_3], axis=-1, name='up_concat_1' + str(1))
            #     # U 2 ci conv
            #     up_2_1 = tf.layers.conv2d(up_2_1, 256, 3, 1, 'same', activation=tf.nn.relu, name="up/conv_1_1")
            #     up_2_1 = tf.layers.conv2d(up_2_1, 256, 3, 1, 'same', activation=tf.nn.relu, name="up/conv_1_2")
            #
            #     is_2_5 = results_score(up_2_1, class_num[1], "instance_5")
            #     up_2_1_out = tf.image.resize_bilinear(is_2_5, [shape[1], shape[2]])
            #     self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_1_out, class_num[1], class_weights[1], name="redisual_layer_5/")
            ...
            #     up_2_2 = tf.layers.conv2d_transpose(inputs=up_2_1, filters=128, kernel_size=3, strides=2, padding='same', name='up/conv2')
            #     up_2_2 = tf.concat([up_2_2, down_2], axis=-1,name='up_concat_2' + str(2))
            #     up_2_2 = tf.layers.conv2d(up_2_2, 128, 3, 1, 'same', activation=tf.nn.relu, name="up/conv_2_1")
            #     up_2_2 = tf.layers.conv2d(up_2_2, 128, 3, 1, 'same', activation=tf.nn.relu, name="up/conv_2_2")
            #     is_2_6 = results_score(up_2_2, class_num[1], "instance_6")
            #     up_2_2_out = tf.image.resize_bilinear(is_2_6, [shape[1], shape[2]])
            #     self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_2_out, class_num[1], class_weights[1], name="redisual_layer_6/")
            #
            #     up_2_3 = tf.layers.conv2d_transpose(inputs=up_2_2, filters=64, kernel_size=3, strides=2, padding='same', name='up/conv3')
            #     up_2_3 = tf.concat([up_2_3, down_1], axis=-1, name='up_concat_3' + str(3))
            #     up_2_3 = tf.layers.conv2d(up_2_3, 64, 3, 1, 'same', activation=tf.nn.relu, name="up/conv_3_1")
            #     up_2_3 = tf.layers.conv2d(up_2_3, 64, 3, 1, 'same', activation=tf.nn.relu, name="up/conv_3_2")
            #     is_2_7 = results_score(up_2_3, class_num[1], "instance_7")
            #     up_2_3_out = tf.image.resize_bilinear(is_2_7, [shape[1], shape[2]])
            #     self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_3_out, class_num[1], class_weights[1], name="redisual_layer_7/")
            #
            ...
            with tf.variable_scope("updown_layer_stage2"):
                ...
                # 2^(k+1) + 1  2^k
                # 1 pyramid
                pyramid_1_7_max = tf.layers.max_pooling2d(down_1, 7, 4, 'same')
                pyramid_1_5_max = tf.layers.max_pooling2d(down_1, 5, 2, 'same')
                pyramid_1_3_conv = tf.layers.conv2d(down_1, down_1.shape[-1], 3, 1, 'same',activation=tf.nn.relu, name='up/pyramid_1_3_conv')
                pyramid_1_5_conv = tf.layers.conv2d(down_1, down_1.shape[-1], 5, 1, 'same',activation=tf.nn.relu, name='up/pyramid_1_5_conv')
                down_1_pyramid = tf.concat([pyramid_1_3_conv, pyramid_1_5_conv], -1)
                down_1_pyramid = tf.layers.conv2d(down_1_pyramid, down_1.shape[-1], 1, 1, 'same', activation=tf.nn.relu, name='up/pyramid_mix_1_1_conv')
                # 2 pyramid
                # last error seting conv last down1.shape
                pyramid_2_5_max = tf.layers.max_pooling2d(down_2, 5, 2, 'same')
                pyramid_2_3_conv = tf.layers.conv2d(down_2, down_2.shape[-1], 3, 1, 'same',activation=tf.nn.relu, name='up/pyramid_2_3_conv')
                pyramid_2_5_conv = tf.layers.conv2d(down_2, down_2.shape[-1], 5, 1, 'same',activation=tf.nn.relu, name='up/pyramid_2_5_conv')
                down_2_pyramid = tf.concat([pyramid_1_5_max, pyramid_2_3_conv, pyramid_2_5_conv], -1)
                down_2_pyramid = tf.layers.conv2d(down_2_pyramid, down_2.shape[-1], 1, 1, 'same',activation=tf.nn.relu, name='up/pyramid_mix_2_1_conv')

                # 3 pyramid
                # last error seting all conv down2.shape
                pyramid_3_3_conv = tf.layers.conv2d(down_3, down_3.shape[-1], 3, 1, 'same',activation=tf.nn.relu, name='up/pyramid_3_3_conv')
                pyramid_3_5_conv = tf.layers.conv2d(down_3, down_3.shape[-1], 5, 1, 'same',activation=tf.nn.relu, name='up/pyramid_3_5_conv')
                down_3_pyramid = tf.concat([pyramid_1_7_max, pyramid_2_5_max, pyramid_3_3_conv, pyramid_3_5_conv], -1)
                down_3_pyramid = tf.layers.conv2d(down_3_pyramid, down_3.shape[-1], 1, 1, 'same', activation=tf.nn.relu, name='up/pyramid_mix_3_1_conv')

                up_2_1 = tf.layers.conv2d_transpose(inputs=down_4, filters=256, kernel_size=3,
                                                    strides=2, padding='same', name='up/conv1')
                up_2_1 = tf.concat([up_2_1, down_3_pyramid], axis=-1, name='up_concat_1' + str(1))
                is_2_5 = results_score(up_2_1, class_num[1], "instance_5")
                up_2_1_out = tf.image.resize_bilinear(is_2_5, [shape[1], shape[2]])
                self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_1_out, class_num[1],
                                                                   class_weights[1], name="redisual_layer_5/")

                up_2_2 = tf.layers.conv2d_transpose(inputs=up_2_1, filters=128, kernel_size=3, strides=2,
                                                    padding='same', name='up/conv2')
                up_2_2 = tf.concat([up_2_2, down_2_pyramid], axis=-1, name='up_concat_2' + str(2))
                is_2_6 = results_score(up_2_2, class_num[1], "instance_6")
                up_2_2_out = tf.image.resize_bilinear(is_2_6, [shape[1], shape[2]])
                self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_2_out, class_num[1],
                                                                   class_weights[1], name="redisual_layer_6/")

                up_2_3 = tf.layers.conv2d_transpose(inputs=up_2_2, filters=64, kernel_size=3, strides=2,
                                                    padding='same', name='up/conv3')
                up_2_3 = tf.concat([up_2_3, down_1_pyramid], axis=-1, name='up_concat_3' + str(3))
                is_2_7 = results_score(up_2_3, class_num[1], "instance_7")
                up_2_3_out = tf.image.resize_bilinear(is_2_7, [shape[1], shape[2]])
                self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_3_out, class_num[1],
                                                                   class_weights[1], name="redisual_layer_7/")

                up_2_4 = tf.layers.conv2d_transpose(up_2_3, filters=32, kernel_size=3, strides=2, padding='same',
                                                    name='up/conv4')
                up_2_4_out = results_score(up_2_4, class_num[1], "instance_8")
                self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_4_out, class_num[1],
                                                                   class_weights[1], name="redisual_layer_8/")
    ...
    def build_multi_parallel_pyramid_net(self, inputs_org, class_num, class_weights, train=True):
        ...
        with tf.variable_scope("stage2"):
            ...
            with tf.variable_scope("layer1"):
                ...
                parallel_4_1 = redisual_block(parallel_3_1, 3, 64, 3, 0, train, 1, "parallel_redisual_layer_4_1")
                p_TD_4_1 = transition_down(parallel_4_1, parallel_4_1.get_shape()[-1], 3, 0, train, 2,
                                           'parallel_transition_down_4_1')
            ...
            with tf.variable_scope("layer2"):
                ...
                p_TD_4_1 = tf.concat([parallel_3_2, p_TD_4_1], axis=-1)
                parallel_4_2 = redisual_block(p_TD_4_1, 3, 128, 3, 0, train, 1, "parallel_redisual_layer_4_2")
                p_TD_4_2 = transition_down(parallel_4_2, parallel_4_2.get_shape()[-1], 3, 0, train, 2,
                                           'parallel_transition_down_4_2')
            ...
            with tf.variable_scope("layer3"):
                ...
                p_TD_4_2 = tf.concat([parallel_3_3, p_TD_4_2], axis=-1)
                parallel_4_3 = redisual_block(p_TD_4_2, 3, 256, 3, 0, train, 1, "parallel_redisual_layer_4_3")
                p_TD_4_3 = transition_down(parallel_4_3, parallel_4_3.get_shape()[-1], 3, 0, train, 2, 'parallel_transition_down_4_3')
            ...
            with tf.variable_scope("layer4"):
                ...
                down_4 = tf.concat([TD_3, p_TD_1_3, p_TD_2_3, p_TD_3_3, p_TD_4_3], axis=-1)
                down_4 = redisual_block(down_4, 3, 256, 3, 0, train, 1, "redisual_layer_4")
                if visualization:
                    self.down_4 = down_4
                #TD_4 = down_4
    ...
    def build_multi_parallel_up_net(self, inputs_org, class_num, class_weights, train=True, targetUpNum = 1):
        visualization = True
        with tf.variable_scope("stage2"):
            # orgin image visualization：
            init = self.init_img(inputs_org, train)
            ...
            with tf.variable_scope("updown_layer_stage2"):
                # layer 1
                up_1_1 = tf.layers.conv2d_transpose(inputs=down_4, filters=256, kernel_size=3, strides=2, padding='same', name='up/conv1')
                up_1_1 = tf.concat([up_1_1, down_3], axis=-1, name='up_concat_1' + str(1))
                up_2_1_r = up_1_1
                if targetUpNum > 1:
                    # p1_1
                    up_2_1 = tf.layers.conv2d(up_1_1, 256, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_1_1')
                    up_2_1_r = up_2_1
                    if targetUpNum > 2:
                        # p1_2
                        up_3_1 = tf.layers.conv2d(up_2_1, 256, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_1_2')
                        up_2_1_r = up_3_1
                        if targetUpNum > 3:
                            up_4_1 = tf.layers.conv2d(up_3_1, 256, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_1_3')
                            up_2_1_r = up_4_1
                            if targetUpNum > 4:
                                up_5_1 = tf.layers.conv2d(up_4_1, 256, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_1_4')
                                up_2_1_r = up_5_1
                is_2_5 = results_score(up_2_1_r, class_num[1], "instance_5")
                up_2_1_out = tf.image.resize_bilinear(is_2_5, [shape[1], shape[2]])
                self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_1_out, class_num[1], class_weights[1], name="redisual_layer_5/")

                # layer 2
                up_1_2 = tf.layers.conv2d_transpose(inputs=up_1_1, filters=128, kernel_size=3, strides=2, padding='same', name='up/conv2')
                up_1_2 = tf.concat([up_1_2, down_2], axis=-1,name='up_concat_2' + str(2))
                up_2_2_r = up_1_2
                if targetUpNum > 1:
                    # p1_1
                    up_2_2 = tf.layers.conv2d_transpose(inputs=up_2_1, filters=128, kernel_size=3, strides=2, padding='same', name='up/conv2_1')
                    up_2_2 = tf.concat([up_2_2, up_1_2], axis=-1, name='up_concat_2_2')
                    up_2_2 = tf.layers.conv2d(up_2_2, 128, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_2_1')
                    up_2_2_r = up_2_2
                    if targetUpNum > 2:
                        # p1_2
                        up_3_2 = tf.layers.conv2d_transpose(inputs=up_3_1, filters=128, kernel_size=3, strides=2, padding='same', name='up/conv2_2')
                        up_3_2 = tf.concat([up_3_2, up_2_2], axis=-1, name='up_concat_2_3')
                        up_3_2 = tf.layers.conv2d(up_3_2, 128, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_2_2')
                        up_2_2_r = up_3_2
                        if targetUpNum > 3:
                            up_4_2 = tf.layers.conv2d_transpose(inputs=up_4_1, filters=128, kernel_size=3, strides=2, padding='same', name='up/conv2_3')
                            up_4_2 = tf.concat([up_4_2, up_3_2], axis=-1, name='up_concat_2_4')
                            up_4_2 = tf.layers.conv2d(up_4_2, 128, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_2_3')
                            up_2_2_r = up_4_2
                            if targetUpNum > 4:
                                up_5_2 = tf.layers.conv2d_transpose(inputs=up_5_1, filters=128, kernel_size=3,strides=2, padding='same', name='up/conv2_4')
                                up_5_2 = tf.concat([up_5_2, up_4_2], axis=-1, name='up_concat_2_5')
                                up_5_2 = tf.layers.conv2d(up_5_2, 128, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_2_4')

                                up_2_2_r = up_5_2
                is_2_6 = results_score(up_2_2_r, class_num[1], "instance_6")
                up_2_2_out = tf.image.resize_bilinear(is_2_6, [shape[1], shape[2]])
                self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_2_out, class_num[1], class_weights[1], name="redisual_layer_6/")

                # layer 3
                up_1_3 = tf.layers.conv2d_transpose(inputs=up_1_2, filters=64, kernel_size=3, strides=2, padding='same', name='up/conv3')
                up_1_3 = tf.concat([up_1_3, down_1], axis=-1, name='up_concat_3' + str(3))
                up_2_3_r = up_1_3
                up_x_all = up_1_3
                if targetUpNum > 1:
                    # p1_1
                    up_2_3 = tf.layers.conv2d_transpose(inputs=up_2_2, filters=64, kernel_size=3, strides=2, padding='same', name='up/conv3_1')
                    up_2_3 = tf.concat([up_2_3, up_1_3], axis=-1, name='up_concat_3_2')
                    up_2_3 = tf.layers.conv2d(up_2_3, 64, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_3_1')
                    up_2_3_r = up_2_3
                    up_x_all = tf.concat([up_x_all, up_2_3], axis=-1, name='up_concat_4_2')
                    if targetUpNum > 2:
                        # p1_2
                        up_3_3 = tf.layers.conv2d_transpose(inputs=up_3_2, filters=64, kernel_size=3, strides=2, padding='same', name='up/conv3_2')
                        up_3_3 = tf.concat([up_3_3, up_2_3], axis=-1, name='up_concat_3_3')
                        up_3_3 = tf.layers.conv2d(up_3_3, 64, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_3_2')
                        up_2_3_r = up_3_3
                        up_x_all = tf.concat([up_x_all, up_3_3], axis=-1, name='up_concat_4_3')
                        if targetUpNum > 3:
                            up_4_3 = tf.layers.conv2d_transpose(inputs=up_4_2, filters=64, kernel_size=3, strides=2,padding='same', name='up/conv3_3')
                            up_4_3 = tf.concat([up_4_3, up_3_3], axis=-1, name='up_concat_3_4')
                            up_4_3 = tf.layers.conv2d(up_4_3, 64, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_3_3')
                            up_2_3_r = up_4_3
                            up_x_all = tf.concat([up_x_all, up_4_3], axis=-1, name='up_concat_4_4')
                            if targetUpNum > 4:
                                up_5_3 = tf.layers.conv2d_transpose(inputs=up_5_2, filters=64, kernel_size=3, strides=2, padding='same', name='up/conv3_4')
                                up_5_3 = tf.concat([up_5_3, up_4_3], axis=-1, name='up_concat_3_5')
                                up_5_3 = tf.layers.conv2d(up_5_3, 64, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_3_4')
                                up_2_3_r = up_5_3
                                up_x_all = tf.concat([up_x_all, up_5_3], axis=-1, name='up_concat_4_5')

                is_2_7 = results_score(up_2_3_r, class_num[1], "instance_7")
                up_2_3_out = tf.image.resize_bilinear(is_2_7, [shape[1], shape[2]])
                self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_3_out, class_num[1], class_weights[1], name="redisual_layer_7/")

                #layer 4
                up_2_4 = tf.layers.conv2d_transpose(up_x_all, filters=32, kernel_size=3, strides=2, padding='same', name='up/conv4')
                up_2_4_out = results_score(up_2_4, class_num[1], "instance_8")
                self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_4_out, class_num[1], class_weights[1], name="redisual_layer_8/")
    ...
    # three down conv
    def build_multi_parallel_pyramid_and_up_net(self, inputs_org, class_num, class_weights, train=True, targetUpNum = 1):
        visualization = True
        with tf.variable_scope("stage2"):
            inputs = self.init_img(inputs_org, train)
            shape = tf.shape(inputs_org)
            ...
            with tf.variable_scope("updown_layer_stage2"):
                ...
                # # 2^(k+1) + 1  2^k
                # 1 pyramid
                #down_1_add = tf.add(tf.add(down_1, parallel_1_1), parallel_2_1)
                #down_1_add = down_1
                down_1_add = parallel_2_1
                pyramid_1_7_max = tf.layers.max_pooling2d(down_1_add, 7, 4, 'same')
                pyramid_1_5_max = tf.layers.max_pooling2d(down_1_add, 5, 2, 'same')
                pyramid_1_3_conv = tf.layers.conv2d(down_1_add, down_1_add.shape[-1], 3, 1, 'same',activation=tf.nn.relu, name='up/pyramid_1_3_conv')
                pyramid_1_5_conv = tf.layers.conv2d(down_1_add, down_1_add.shape[-1], 5, 1, 'same',activation=tf.nn.relu, name='up/pyramid_1_5_conv')
                down_1_pyramid = tf.concat([pyramid_1_3_conv, pyramid_1_5_conv], -1)
                down_1_pyramid = tf.layers.conv2d(down_1_pyramid, down_1_add.shape[-1], 1, 1, 'same', activation=tf.nn.relu, name='up/pyramid_mix_1_1_conv')
                # 2 pyramid
                #down_2_add = tf.add(tf.add(down_2, parallel_1_2), parallel_2_2)
                #down_2_add = down_2
                down_2_add = parallel_2_2
                pyramid_2_5_max = tf.layers.max_pooling2d(down_2_add, 5, 2, 'same')
                pyramid_2_3_conv = tf.layers.conv2d(down_2_add, down_2_add.shape[-1], 3, 1, 'same',activation=tf.nn.relu, name='up/pyramid_2_3_conv')
                pyramid_2_5_conv = tf.layers.conv2d(down_2_add, down_2_add.shape[-1], 5, 1, 'same',activation=tf.nn.relu, name='up/pyramid_2_5_conv')
                down_2_pyramid = tf.concat([pyramid_1_5_max, pyramid_2_3_conv, pyramid_2_5_conv], -1)
                down_2_pyramid = tf.layers.conv2d(down_2_pyramid, down_2_add.shape[-1], 1, 1, 'same',activation=tf.nn.relu, name='up/pyramid_mix_2_1_conv')
                # 3 pyramid
                #down_3_add = tf.add(tf.add(down_3, parallel_1_3), parallel_2_3)
                #down_3_add = down_3
                down_3_add = parallel_2_3
                pyramid_3_3_conv = tf.layers.conv2d(down_3_add, down_3_add.shape[-1], 3, 1, 'same',activation=tf.nn.relu, name='up/pyramid_3_3_conv')
                pyramid_3_5_conv = tf.layers.conv2d(down_3_add, down_3_add.shape[-1], 5, 1, 'same',activation=tf.nn.relu, name='up/pyramid_3_5_conv')
                down_3_pyramid = tf.concat([pyramid_1_7_max, pyramid_2_5_max, pyramid_3_3_conv, pyramid_3_5_conv], -1)
                down_3_pyramid = tf.layers.conv2d(down_3_pyramid, down_3_add.shape[-1], 1, 1, 'same', activation=tf.nn.relu, name='up/pyramid_mix_3_1_conv')

                # direct concat
                # down_1_pyramid = parallel_2_1
                # down_2_pyramid = parallel_2_2
                # down_3_pyramid = parallel_2_3

                # layer 1
                up_1_1 = tf.layers.conv2d_transpose(inputs=down_4, filters=256, kernel_size=3, strides=2, padding='same', name='up/conv1')
                up_1_1 = tf.concat([up_1_1, down_3_pyramid], axis=-1, name='up_concat_1' + str(1))
                up_2_1_r = up_1_1
                if targetUpNum > 1:
                    # p1_1
                    up_2_1 = tf.layers.conv2d(up_1_1, 256, 3, 1, 'same', activation=tf.nn.relu, name='up/conv1_p_1_1')
                ...
                # layer 2
                up_1_2 = tf.layers.conv2d_transpose(inputs=up_1_1, filters=128, kernel_size=3, strides=2, padding='same', name='up/conv2')
                up_1_2 = tf.concat([up_1_2, down_2_pyramid], axis=-1,name='up_concat_2' + str(2))
                up_2_2_r = up_1_2
                if targetUpNum > 1:
                    # p1_1
                    up_2_2 = tf.layers.conv2d_transpose(inputs=up_2_1, filters=128, kernel_size=3, strides=2, padding='same', name='up/conv2_1')
                ...
                # layer 3
                up_1_3 = tf.layers.conv2d_transpose(inputs=up_1_2, filters=64, kernel_size=3, strides=2, padding='same', name='up/conv3')
                up_1_3 = tf.concat([up_1_3, down_1_pyramid], axis=-1, name='up_concat_3' + str(3))
                up_2_3_r = up_1_3
                up_x_all = up_1_3
                if targetUpNum > 1:
                    # p1_1
    ...
    def build_net_new(self, inputs_org, class_num, class_weights, train=True):
        ...
        with tf.variable_scope("stage1"):
            ...
            with tf.variable_scope("layer4"):
                ...
                down_4 = res_block(TD_3, 2, 512, 2048, 3, train, name="redisual_layer_4")
                #TD_4 = down_4
                #self.mid_result.append(down_4)
                #TD_4 = results_score(TD_4, 32, "pre_instance_4")
                # is4 = tf.image.resize_bilinear(TD_4, [shape[1], shape[2]])
            ...
            with tf.variable_scope("updown_layer_stage"):
                ...
                up_2_1 = tf.concat([up_2_1, down_3], axis=-1, name='up_concat_1' + str(1))
                # U 2 ci conv
                up_2_1 = tf.layers.conv2d(up_2_1, 1024, 3, 1, 'same', activation=tf.nn.relu, name="up/conv_1_1")
                up_2_1 = tf.layers.conv2d(up_2_1, 1024, 3, 1, 'same', activation=tf.nn.relu, name="up/conv_1_2")
                ...
                up_2_2 = tf.layers.conv2d_transpose(inputs=up_2_1, filters=512, kernel_size=3, strides=2, padding='same', name='up/conv2')
                up_2_2 = tf.concat([up_2_2, down_2], axis=-1,name='up_concat_2' + str(2))
                up_2_2 = tf.layers.conv2d(up_2_2, 512, 3, 1, 'same', activation=tf.nn.relu, name="up/conv_2_1")
                up_2_2 = tf.layers.conv2d(up_2_2, 512, 3, 1, 'same', activation=tf.nn.relu, name="up/conv_2_2")
                is_2_6 = results_score(up_2_2, class_num[1], "instance_6")
                up_2_2_out = tf.image.resize_bilinear(is_2_6, [shape[1], shape[2]])
                self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_2_out, class_num[1], class_weights[1], name="redisual_layer_6/")
                ...
                up_2_3 = tf.layers.conv2d_transpose(inputs=up_2_2, filters=256, kernel_size=3, strides=2, padding='same', name='up/conv3')
                up_2_3 = tf.concat([up_2_3, down_1], axis=-1, name='up_concat_3' + str(3))
                up_2_3 = tf.layers.conv2d(up_2_3, 256, 3, 1, 'same', activation=tf.nn.relu, name="up/conv_3_1")
                up_2_3 = tf.layers.conv2d(up_2_3, 256, 3, 1, 'same', activation=tf.nn.relu, name="up/conv_3_2")
                is_2_7 = results_score(up_2_3, class_num[1], "instance_7")
                up_2_3_out = tf.image.resize_bilinear(is_2_7, [shape[1], shape[2]])
                self.loss += dynamic_sparse_weighted_cross_entropy(self.output, up_2_3_out, class_num[1], class_weights[1], name="redisual_layer_7/")
