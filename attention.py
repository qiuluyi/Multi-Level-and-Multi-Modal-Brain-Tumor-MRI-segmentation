...
def conv(x, channels, kernel=4, stride=1, pad=0, pad_type='zero', use_bias=False, sn=True, scope='conv_0'):
    with tf.variable_scope(scope):
        ...
        if sn:
            ...
            w = tf.get_variable("kernel", shape=[kernel, kernel, x.get_shape()[-1], channels], initializer=weight_init,
                                    regularizer=weight_regularizer,trainable = True)
                # w = tf.Variable(tf.truncated_normal_initializer([kernel, kernel, x.get_shape()[-1], channels],mean=0.0, stddev=0.02))
            x = tf.nn.conv2d(input=x, filter=spectral_norm(w),
                                 strides=[1, stride, stride, 1], padding='SAME')
            # if x.get_shape()[-1] == 128:
            #     w = tf.get_variable("kernel2", shape=[kernel, kernel, x.get_shape()[-1], channels],
...
def atblock(x, channels, use_bias=True, sn=False, scope='at_block'):
    with tf.variable_scope(scope):
        with tf.variable_scope('context_modeling'):
            ...
            context_mask = conv(x, channels=1, kernel=3, stride=1, use_bias=use_bias, sn=sn, scope='conv')
        ...
        with tf.variable_scope('transform_0'):
            context_transform = conv(context, channels, kernel=3, stride=1, use_bias=use_bias, sn=sn, scope='conv_0')
            context_transform = layer_norm(context_transform)
            context_transform = relu(context_transform)
            context_transform = conv(context_transform, channels=c, kernel=3, stride=1, use_bias=use_bias, sn=sn, scope='conv_1')
