...
class model:
    def __init__(self, img_1, img_2, out_1, out_2, class_num=[2,5], class_weights =[1, 5, 3, 4, 10],
        ...
        self.input_1 = img_1
        self.input_2 = img_2
        # self.input_3 = tf.concat((img_1, img_2), axis=-1)
        self.output_1 = out_1
        self.output_2 = out_2
        ...
        self.logits_2 = self.build_net_2(self.input_1, self.input_2, class_num, class_weights, train)
    ...
    def build_fuzzy_feature(self, inputs_org):
        flair = inputs_org[:, :, :, 0]
        t1 = inputs_org[:, :, :, 1]
        t1c = inputs_org[:, :, :, 2]
        t2 = inputs_org[:, :, :, 3]

        flair_1 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(flair, 3.275618700192281)), 1.9394266340981592))
        t1_1 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(t1, 1.7127770276396648)), 0.3387410599660966))
        t1c_1 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(t1c, 1.4795579926659643)), 0.3279158653497632))
        t2_1 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(t2, 4.694130640832885)), 1.3942960886622808))
        score_1 = tf.reduce_min(tf.stack([flair_1, t1_1, t1c_1,t2_1], axis=-1), axis=-1)

        flair_2 = tf.exp( tf.div(-1.0 * tf.square(tf.subtract(flair, 3.5771133851663452)), 0.7775774317247557))
        t1_2 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(t1, 2.358920554602273)), 0.22036718798863333))
        t1c_2 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(t1c, 2.215823311998001)), 0.24690377056698246))
        t2_2 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(t2, 2.847848183105326)), 0.5800377854115898))
        score_2 = tf.reduce_min(tf.stack([flair_2, t1_2, t1c_2, t2_2], axis=-1), axis=-1)

        flair_3 = tf.exp( tf.div(-1.0 * tf.square(tf.subtract(flair, 3.6790987931443677)), 1.5599086870326495))
        t1_3 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(t1, 2.0689728087702597)), 0.5605638159165642))
        t1c_3 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(t1c, 2.2323669819492222)), 0.8592803794743282))
        t2_3 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(t2, 3.5364416434375334)), 1.3217699033346926))
        score_3 = tf.reduce_min(tf.stack([flair_3, t1_3, t1c_3, t2_3], axis=-1), axis=-1)

        flair_4 = tf.exp( tf.div(-1.0 * tf.square(tf.subtract(flair, 3.874380818559149)), 1.5333211250554655))
        t1_4 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(t1, 2.352193680642174)), 0.47007257442133893))
        t1c_4 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(t1c, 3.797870028823101)), 1.0521635863820364))
        t2_4 = tf.exp(tf.div(-1.0 * tf.square(tf.subtract(t2, 3.359715404451498)), 0.9212955989084565))
        score_4 = tf.reduce_min(tf.stack([flair_4, t1_4, t1c_4, t2_4], axis=-1), axis=-1)

        return tf.stack([score_1, score_2, score_3, score_4], axis=-1)
    ...
    # @staticmethod
    def build_net_2(self, inputs_org, inputs_fuzzy, class_num, class_weights, train=True):
        with tf.variable_scope("stage2"):

            inputs = self.init_img(inputs_org, train, name="inputs_org_init")

            inputs_fuzzy = self.init_img(inputs_fuzzy, train, name="inputs_fuzzy_init")
            ...
            with tf.variable_scope("layer1"):
                ...
                # down_1_f = redisual_block(inputs_fuzzy, 3, 64, 3, 0, train, 1, "redisual_layer_1_fuzzy")

                # down_1_temp = tf.multiply(down_1,down_1_f)
                # down_1 = tf.add(down_1_temp, down_1)
                # down_1 = tf.concat([down_1,down_1_f],axis=-1)

                TD_1 = transition_down(down_1, down_1.get_shape()[-1], 3, 0, train, 2, 'transition_down_1')

                fuzzy_1, TD_1 = fuzzy_attention(inputs_fuzzy, TD_1, 64, 5, 2)

                # TD_1_f = transition_down(down_1_f, down_1_f.get_shape()[-1], 3, 0, train, 2, 'transition_down_1_fuzzy')
            ...
            with tf.variable_scope("layer2"):
                down_2 = redisual_block(TD_1, 3, 128, 3, 0, train, 1, "redisual_layer_2")
                # fuzzy_attention
                # down_2_f = redisual_block(TD_1_f, 3, 128, 3, 0, train, 1, "redisual_layer_2_fuzzy")
                # down_2_temp = tf.multiply(down_2, down_2_f)
                # down_2 = tf.add(down_2_temp, down_2)
                # down_2 = tf.concat([down_2, down_2_f], axis=-1)

                TD_2 = transition_down(down_2, down_2.get_shape()[-1], 3, 0, train, 2, 'transition_down_2')

                fuzzy_2, TD_2 = fuzzy_attention(fuzzy_1, TD_2, 128, 5, 2)

                # TD_2_f = transition_down(down_2_f, down_2_f.get_shape()[-1], 3, 0, train, 2, 'transition_down_2_fuzzy')
            ...
            with tf.variable_scope("layer3"):
                ...
                # fuzzy_attention
                # down_3_f = redisual_block(TD_2_f, 3, 256, 3, 0, train, 1, "redisual_layer_3_fuzzy")
                # down_3_temp = tf.multiply(down_3, down_3_f)
                # down_3 = tf.add(down_3_temp, down_3)
                # down_3 = tf.concat([down_3, down_3_f], axis=-1)

                TD_3 = transition_down(down_3, down_3.get_shape()[-1], 3, 0, train, 2, 'transition_down_3')
                fuzzy_3, TD_3 = fuzzy_attention(fuzzy_2, TD_3, 256, 5, 2)

                # TD_3_f = transition_down(down_3_f, down_3_f.get_shape()[-1], 3, 0, train, 2, 'transition_down_3_fuzzy')
            ...
            with tf.variable_scope("layer4"):
                ...
                # fuzzy_attention
                # down_4_f = redisual_block(TD_3_f, 3, 256, 3, 0, train, 1, "redisual_layer_4_fuzzy")
                # down_4_temp = tf.multiply(down_4, down_4_f)
                # down_4 = tf.add(down_4_temp, down_4)
                # down_4 = tf.concat([down_4, down_4_f], axis=-1)
    ...
    @staticmethod
    def init_img(inputs, train=True, name="init"):
